...
<div class="panel panel-default">
    <div class="panel-heading">
      <h3 class="panel-title">Para interagir com um chamado, clique no botão responder:</h3>
    </div>
...
$busca=$pdo->prepare("SELECT A.CHA_ID, A. CAT_ID, A.CHA_ASSUNTO, A.CHA_DESCRICAO, A.CHA_STATUS, A.CHA_SOLICITANTE, A.CHA_RESPONSAVEL, A.CHA_DT_ABERTURA, A.CHA_DT_SOLUCAO, B.CAT_NOME, C.STA_NOME
                      ...
                      WHERE A.CAT_ID = B.CAT_ID AND A.CHA_STATUS = C.STA_ID AND A.CHA_STATUS = 0  AND A.CHA_SOLICITANTE = '$usuario'
                      ORDER BY 1");
$busca->execute();
    ...
    while ($obj=$busca->fetch(PDO::FETCH_OBJ)){
      ...
      echo("<tr>
          ...
          <td class='last-td' style='width: 110px'>
            <form method='POST' action='res_chamado_usu_f.php'>
              <input type='hidden' name='alterar' value='$id'>
              <button type='submit' class='btn btn-success btn-sm'><span class='glyphicon glyphicon-search' aria-hidden='true'></span> Visualizar</button>
            </form>
          </td>
        </tr>");
    }
